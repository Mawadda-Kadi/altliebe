...
class RegisterTestCase(TestCase):
    ...
    def test_register_success(self):
        ...
        self.assertTrue(
            user_model.objects.filter(username='testuser').exists())

        # Check that user is authenticated and redirect them to profile page
        self.assertRedirects(
            response, reverse('user-profile', kwargs={'username': 'testuser'}))
...
class UserProfileViewTest(TestCase):
    ...
    def test_view_own_profile(self):
        ...
        response = self.client.get(
            reverse('user-profile', kwargs={'username': self.user.username}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_view_other_profile(self):
        ...
        response = self.client.get(
            reverse('user-profile', kwargs={'username': other_user.username}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_profile_not_found(self):
        ...
        response = self.client.get(
            reverse('user-profile', kwargs={'username': 'nonexistent'}))
